...
from solrstice.clients import BlockingSolrCloudClient, AsyncSolrCloudClient
...
def test_blocking_client_does_not_block_gil_config(config: Config):
    ...
    with ThreadPool(processes) as pool:
        ...
        tasks = [
            pool.apply_async(get_configs_blocking, (config.speedbump_host, config.solr_username, config.solr_password))
...
async def test_async_client_does_not_block_event_loop(config: Config):
    ...
    if not config.speedbump_host:
        pytest.skip("No speedbump host configured")
    start_time = time.time()
    tasks = [asyncio.create_task(get_configs_async(config.speedbump_host, config.solr_username, config.solr_password))
             for _ in range(processes)]
...
def get_configs_blocking(host: str, username: Optional[str], password: Optional[str]):
    ...
    client = BlockingSolrCloudClient(SolrServerContext(SolrSingleServerHost(host), auth))
    return client.get_configs()


async def get_configs_async(host: str, username: Optional[str], password: Optional[str]):
    auth = None if not username else SolrBasicAuth(username, password)
